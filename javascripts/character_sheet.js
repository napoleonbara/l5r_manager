...
(function() {
  var CharacterSheet, Expression, MAPPING, RESERVED_SYMBOLS, Roll, SKILLS, StringChunk, add, div, mul, parse, sub,
    __slice = [].slice,
  ...
  $(function() {
    ...
    $('#dice_roller input[type=button]').click(function() {
      var err, handle_query, input, message, out, result, summary, _ref;
      ...
      if (input.length) {
        try {
          _ref = handle_query(input), result = _ref[0], summary = _ref[1];
          switch (false) {
            case typeof result !== 'number':
              return put_number_result(input, result, summary);
            case result.mode !== 'L5R':
              return roll_l5r(input, result, summary);
            case result.mode !== 'basic':
              return roll_basic(input, result, summary);
          }
        } catch (_error) {
          err = _error;
          message = (function() {
    ...
    put_number_result = function(input, result, summary) {
      var out;
      out = $("#dice_result");
      return out.html("<div id='summary'>" + input + " = " + result + "<div>");
    };
    roll_basic = function(input, result, summary) {
    ...
    roll_l5r = function(input, result, summary) {
      ...
      out.html("<div id='summary'>" + summary + ":<div>              <div id='thrown'>" + (result.to_string()) + ":<div>              <table><tr></tr></table>              <div id='dices_sum'><div>");
      row = out.find("tr");
      for (i = _i = 0; 0 <= roll_num ? _i < roll_num : _i > roll_num; i = 0 <= roll_num ? ++_i : --_i) {
  ...
  CharacterSheet = (function() {
    ...
    CharacterSheet.prototype.complete = function() {
      var context, expression, trait, val, _i, _len, _ref;
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        val = (function() {
          switch (trait) {
            ...
            default:
              expression = window.handle_query(this.mapping[trait].compute);
    ...
    CharacterSheet.prototype.handle_query = function(exp) {
      ...
      return [r.evaluate(this.evaluation_context()), r.to_string()];
    };
  ...
  window.handle_query = function(expression) {
    return new Expression(expression);
  };

  StringChunk = (function() {
    function StringChunk(content, operator) {
      this.content = content;
      this.operator = operator;
      this.precedence = this.get_precedence(this.operator);
    }

    StringChunk.prototype.get_precedence = function(operator) {
      switch (operator) {
        case null:
          return 0;
        case '+':
    ...
    StringChunk.prototype.string = function(operator) {
      var precedence;
      if (operator == null) {
        operator = null;
      }
      precedence = this.get_precedence(operator);
    ...
    Expression.prototype.to_string = function() {
      return this.scan({
        add: function(copy, stack, top, args) {
          return stack.push(new StringChunk("" + (args[1].string('+')) + " + " + (args[0].string('+')), '+'));
        },
        ...
        function_call: function(copy, stack, top, args) {
          ...
          args = stack.splice(i);
          args.shift();
          return stack.push(new StringChunk("" + (func.string('primary')) + "(" + (args.map(function(e) {
            return e.string('primary');
          }).join(', ')) + ")", 'primary'));
        },
        ...
        pool_roll: function(copy, stack, top, args) {
          return stack.push(new StringChunk("" + (args[1].string('D')) + "D" + (args[0].string('D')), 'D'));
        },
        roll_and_keep: function(copy, stack, top, args) {
          return stack.push(new StringChunk("" + (args[1].string('K')) + "K" + (args[0].string('K')), 'K'));
        },
        symbol: function(copy, stack, top, args) {
          return stack.push(new StringChunk(top, 'primary'));
        },
        ...
        explode_flag: function(copy, stack, top, args) {
          return copy.push(new StringChunk("!" + (args[0].string('!'))));
        }
      }).string();
    };
